...
get.rho550 <- function (thetaV, delta.phi, windspeed,thetaS){
  ...
  for (i in 1:(length(xwindspeed)-1)) {
    if (windspeed > xwindspeed[i] & windspeed < xwindspeed[i+1]) {
      rho.min = approx3d(xthetaV, xdelta.phi,xthetaS, rho550[,,i,], thetaV, delta.phi,thetaS)
      rho.max = approx3d(xthetaV, xdelta.phi,xthetaS, rho550[,,i+1,], thetaV,delta.phi,thetaS)
      f = (windspeed - xwindspeed[i]) / (xwindspeed[i+1]-xwindspeed[i])
      rho = rho.min * (1-f) + rho.max * f
      return(rho)
    }
    if (windspeed == xwindspeed[i]){
      rho = approx3d(xthetaV, xdelta.phi,xthetaS, rho550[,,i,], thetaV,delta.phi,thetaS)
      return(rho)
    }
  }
